              ...
              <li key={data.id} className={` my-3  `}>
                {!autoCollapse ? (
                  <NavLink
                    to={data.link}
                    key = {data.id}
                    className={`text-light flex gap-4 py-[20px]  bg-no-repeat bg-cover bg-center px-[10px] delay-500 duration-800 ease-in-out hover:bg-navbg initial-font`}
                    onClick={toggleHandler}
                  >
                    <img src={data.icon} alt="navlink-icon" />
                    {data.title}
                  </NavLink>
                ) : (
                    <NavLink
      ...
      <div className="relative flex flex-col gap-[50px] h-[100vh] ">
        ...
        {!autoCollapse ? (
          <>
            <button
              type="button"
              className=" cursor-pointer lg:hidden absolute top-[8%] right-[-30px] "
              onClick={toggleHandler}
            >
              <svg
                width="40"
